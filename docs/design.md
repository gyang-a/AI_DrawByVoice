...
| 保存/加载作品 | 保存当前画布并恢复 | 未完成 |
| 导出图片 | 导出 PNG/SVG 等文件 | 未完成 |
...
未完成。当前 scene 只保存在前端内存状态中。保存/加载需要设计作品数据结构、存储位置和恢复策略。

### 导出图片

未完成。当前 Konva 画布可以作为导出基础，但还没有实现导出按钮、导出格式选择和 SVG/Canvas 混合内容的完整验证。
