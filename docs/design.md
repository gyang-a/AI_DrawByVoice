...
| 选择/拖拽编辑 | 用户直接在画布上选择、拖拽、缩放对象 | 未实现 |
...
未完成。当前项目强调纯语音指令驱动，尚未实现鼠标选择、拖拽、缩放、旋转控制点等交互，计划作为扩展内容实现。

### 更稳定的复杂引用理解

部分实现。当前模型会收到 scene 并可根据 id、类型、位置和 parts 选择目标，但复杂引用仍依赖模型理解能力，例如“把左边第二个小图标的眼睛变亮”这类指令还需要更强的目标选择策略，后续计划思考更好的架构。
